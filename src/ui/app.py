...
if docs:
    ...
    query1 = st.text_input("您心目中的理專，應該具備哪些特質或特長？", placeholder="範例：我在尋找一位能為高資產客群進行退休規劃，且溝通風格溫和的理專。")
    query2 = st.text_input("描述您滿意或不滿意的服務經歷：", placeholder="範例：之前的理專很少主動聯繫，常常錯過市場機會。")
    
    combined_query = ""
    if query1:
        combined_query += f"理想特質：{query1}\n"
    if query2:
        combined_query += f"服務經歷：{query2}"
