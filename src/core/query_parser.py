    ...
    def parse_query(self, raw_query: str) -> ParsedUserNeeds:
        prompt = PromptTemplate(
            template=(
                ...
                "如果查詢太過模糊、曖昧，或未提及特定的客群/專業領域，請將專業領域等欄位保留為空或 null。\n"
                "特別注意：若使用者提到了特定的地點或分行（如：新北、台北），請將其填入 branch_needed 欄位。\n"
                "【重要判斷】：請判定此查詢「是否屬於尋找理財顧問、金融服務或投資規劃的範疇」。如果使用者只是在找伴侶、聊天、問不相關的常識，請將 is_relevant 設為 False，"
